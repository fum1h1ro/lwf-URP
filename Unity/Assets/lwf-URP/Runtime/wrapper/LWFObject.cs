...
using ProgramObjectConstructor = System.Func<LWF.ProgramObject, int, int, int, LWF.Renderer>;
...
public class LWFObject : MonoBehaviour
{
    ...
    public TextAsset SrcAsset;
    public Texture2D SrcTexture;
	[HideInInspector] [NonSerialized] public bool isAlive = true;
	...
	protected RendererType rendererType = RendererType.CombinedMeshRenderer;
	protected bool executed;
	protected LWFCallbacks lwfLoadCallbacks = new LWFCallbacks();
	protected LWFCallbacks lwfDestroyCallbacks = new LWFCallbacks();
    ...
    {
        if (SrcAsset != null && SrcTexture != null)
        {
            LoadAsset(SrcAsset, SrcTexture);
        }
    }
	...
	public virtual bool LoadAsset(TextAsset path,
		Texture2D texturePrefix, string fontPrefix = "",
		...
		string shaderName = "Universal Render Pipeline/2D/LWF"
		)
	{
		callUpdate = autoUpdate;
		if (inputCamera == null) inputCamera = Camera.main;

		if (lwfLoadCallback != null) lwfLoadCallbacks.Add(lwfLoadCallback);
		if (lwfDestroyCallback != null) lwfDestroyCallbacks.Add(lwfDestroyCallback);

        var data = new LWF.Data(path.bytes);
	...
	public virtual void UpdateLWF(float tick)
	{
		if (lwf == null)
			return;
	...
	{
		if (!callUpdate || lwf == null) return;
		UpdateLWF((Application.isPlaying)? Time.deltaTime : 0);
	}
